	...
	for addr, account := range x {
		if yaccount, found := y[addr]; !found {
			...
			if yaccount.Equal(account) {
				...
				for key, value := range account.Storage {
					if yvalue, found := y[addr].Storage[key]; !found || yvalue != value {
...
func NewSubstateMessage(msg *types.Message) *SubstateMessage {
	...
	smsg.AccessList = types.AccessList{}

	smsg.GasFeeCap = &big.Int{}
	smsg.GasTipCap = &big.Int{}
...
func (msg *SubstateMessage) AsMessage() types.Message {
	return types.NewMessage( msg.From, msg.To, msg.Nonce, msg.Value, msg.Gas, msg.GasPrice, msg.Data, msg.CheckNonce)
}
