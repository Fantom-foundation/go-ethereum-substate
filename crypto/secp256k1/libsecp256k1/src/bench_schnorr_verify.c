...
static void benchmark_schnorr_init(void* arg) {
    ...
    for (k = 0; k < data->numsigs; k++) {
        ...
        CHECK(geth_secp256k1_ec_pubkey_create(data->ctx, &pubkey, data->sigs[k].key));
        CHECK(geth_secp256k1_ec_pubkey_serialize(data->ctx, data->sigs[k].pubkey, &data->sigs[k].pubkeylen, &pubkey, SECP256K1_EC_COMPRESSED));
    }
}
...
static void benchmark_schnorr_verify(void* arg) {
    ...
    for (i = 0; i < 20000 / data->numsigs; i++) {
        ...
        CHECK(geth_secp256k1_ec_pubkey_parse(data->ctx, &pubkey, data->sigs[0].pubkey, data->sigs[0].pubkeylen));
...
int main(void) {
    ...
    data.ctx = geth_secp256k1_context_create(SECP256K1_CONTEXT_SIGN | SECP256K1_CONTEXT_VERIFY);

    data.numsigs = 1;
    run_benchmark("schnorr_verify", benchmark_schnorr_verify, benchmark_schnorr_init, NULL, &data, 10, 20000);

    geth_secp256k1_context_destroy(data.ctx);
    return 0;
}
